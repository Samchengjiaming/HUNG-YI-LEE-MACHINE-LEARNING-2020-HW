...
adagrad_w3=np.zeros([9*18,1])
loss_list=list()
for epoch in range(epoch_nums):
    loss=np.sqrt(np.sum(np.power((y_true-(np.dot(w1,x_1)+np.dot(w2,x_2)+np.dot(w3,x_3))),2))/5751)
    if epoch%10==0:
        print('loss:',loss)
        loss_list.append(loss)
    gradient_w1_b=-2*np.dot(x_1,(y_true-(np.dot(w1,x_1)+np.dot(w2,x_2)+np.dot(w3,x_3))).transpose())
    gradient_w2 =-2*np.dot(x_2,(y_true-(np.dot(w1,x_1)+np.dot(w2,x_2)+np.dot(w3,x_3))).transpose())
    gradient_w3 =-2*np.dot(x_3,(y_true-(np.dot(w1,x_1)+np.dot(w2,x_2)+np.dot(w3,x_3))).transpose())
    adagrad_w1+=gradient_w1_b**2
    adagrad_w2+=gradient_w2**2
    adagrad_w3+=gradient_w3**2
    ...
    w3=w3-(learning_rate*gradient_w3/np.sqrt(adagrad_w3+esp)).reshape(1,162)
